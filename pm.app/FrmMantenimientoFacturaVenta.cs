...
namespace pm.app
{
    public partial class FrmMantenimientoFacturaVenta : RadForm
    {
        ...
        {
            FacturaVentaBe item = facturaVentaBl.ObtenerFacturaVenta(codigoFacturaVenta.Value, true, true, withUnidadMedida: true);
        ...
        private void btnGuardar_Click(object sender, EventArgs e)
        {
            ...
            bool seGuardoRegistro = facturaVentaBl.GuardarFacturaVenta(registro, out nroComprobante, out totalEnLetras);
            ...
            if (seGuardoRegistro)
            {
                ...
                if (Emision.ExisteConexaoInternet() == true)
                {
                    ...
                    //Genera el xml
                    DocumentoFactElectronicos(registro, out archivoXML);
                    if (!string.IsNullOrEmpty(archivoXML))
                    {
                        ...
                        seGuardoRegistro = facturaVentaBl.GuardarEmisionFacturaVenta(registro);
                    }
                }
                else
                {
        ...
        //FACTURACIÓN
        void DocumentoFactElectronicos(FacturaVentaBe item, out string archivoXML)
        {
            archivoXML = null;
            ...
            Fact_XML.sac = EspacioNombres.sac;
            ...
            Leyenda.Value = item.TotalEnLetras;//ASIGNAR MONTO EN LETRAS
            Leyendas[0] = Leyenda;
            ...
            //'-----------------------------detallle---------------------------------------------------'
            int contador = item.ListaFacturaVentaDetalle.Count + 1;
        ...
        string GenerarComprobanteFactura(ref InvoiceType FactXML, string EmpresaRuc, string TipoFactura, string Fac_serie, string NumFactura)
        {
            ...
            setting.Indent = true;
            setting.IndentChars = "\t";
            string RutaDocumento = Path.Combine(configuracionValor.RutaFacturacionElectronica, "FACTURAS");

            //if (TipoFactura == "01")
            //{
                //RutaDocumento = Path.Combine(configuracionValor.RutaFacturacionElectronica, "FACTURAS");// @"D:\FACTURA ELECTRONICA\FACTURAS\";
            //}
            //else if (TipoFactura == "03")
            //{
            //    RutaDocumento = @"D:\FACTURA ELECTRONICA\BOLETAS\";
            //    //}
            //    //else if (TipoFactura == "07")
            //    //{
            //    //    RutaDocumento = @"D:\FACTURA ELECTRONICA\NOTA CREDITO\";
            //};
            ...
            //  text = text.Replace("</ext:UBLExtension>", "</ext:UBLExtension>" + "\t" + "<ext:UBLExtension>" + "\t" + "<ext:ExtensionContent>" + "\t" + "</ext:ExtensionContent>" + "\t" + "</ext:UBLExtension>");
        ...
        public void EnviarDocumento(string FileArchivo, string ArchXML, out string CodigoHashRpta, out string DescripcionRpta, out string CodigoRpta)
        {
            ...
            try
            {
                ...
                //facturaVentaBl.ModificarHast(registro);
                //BuscarFacturasVenta();
