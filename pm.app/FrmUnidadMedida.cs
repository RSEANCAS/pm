...
namespace pm.app
{
    public partial class FrmUnidadMedida : RadForm
    {
        ...
        void BuscarUnidadesMedida()
        {
            string descripcion = txtFiltroDescripcion.Text.Trim();
            bool flagActivo = chkActivo.Checked;

            List<UnidadMedidaBe> resultados = unidadMedidaBl.BuscarUnidadMedida(descripcion, flagActivo);
        ...
        {
            FrmMantenimientoUnidadMedida frm = new FrmMantenimientoUnidadMedida();
            frm.ShowInTaskbar = false;
            frm.BringToFront();
            DialogResult dr = frm.ShowDialog();
            if (dr == DialogResult.OK) BuscarUnidadesMedida();
        }
        ...
        private void dgvResultados_MouseClick(object sender, MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Right)
            {
                ...
                int codigoUnidadMedida = (int)dgvResultados.Rows[currentMouseOverRow].Cells["dgvResultados_CodigoUnidadMedida"].Value;

                ContextMenu m = new ContextMenu();
                MenuItem mitEditar = new MenuItem("Editar", mitEditar_Click);
                mitEditar.Tag = codigoUnidadMedida;

                MenuItem mitToggleActivar = new MenuItem(flagActivo ? "Inactivar" : "Activar", mitToggleActivar_Click);
                mitToggleActivar.Tag = new { CodigoUnidadMedida = codigoUnidadMedida, FlagActivo = flagActivo };
        ...
        private void mitEditar_Click(object sender, EventArgs e)
        {
            ...
            int codigoUnidadMedida = (int)mitControl.Tag;

            FrmMantenimientoUnidadMedida frm = new FrmMantenimientoUnidadMedida(codigoUnidadMedida);
            frm.ShowInTaskbar = false;
            frm.BringToFront();
            DialogResult dr = frm.ShowDialog();
            if (dr == DialogResult.OK) BuscarUnidadesMedida();
        ...
        private void mitToggleActivar_Click(object sender, EventArgs e)
        {
            ...
            if (dr == DialogResult.Yes)
            {
                ...
                registro.CodigoUnidadMedida = data.CodigoUnidadMedida;
                registro.FlagActivo = !data.FlagActivo;
                bool seGuardo = unidadMedidaBl.CambiarFlagActivoUnidadMedida(registro);
                if (seGuardo)
                {
