...
namespace pm.app
{
    public partial class FrmMantenimientoNotaDebito : RadForm
    {
        int? codigoNotaDebito;
        ...
        List<NotaDebitoDetalleBe> listaDetalleInicial = new List<NotaDebitoDetalleBe>();
        List<NotaDebitoDetalleBe> listaDetalle = new List<NotaDebitoDetalleBe>();
        ...
        public FrmMantenimientoNotaDebito(int? codigoNotaDebito = null, dynamic comprobanteRef = null, TipoComprobante? tipoComprobante = null)
        {
            InitializeComponent();
        ...
        private void FrmMantenimientoNotaDebito_Load(object sender, EventArgs e)
        {
            configuracionValor = configuracionValorBl.ObtenerConfiguracionValor();
            Text = !codigoNotaDebito.HasValue ? "Nueva Nota de Débito" + (comprobanteRef == null ? "" : $" - {tipoComprobante.GetAttributeOfType<DescriptionAttribute>().Description} {comprobanteRef.Serie.Serial} - {comprobanteRef.NroComprobante.ToString("00000000")}") : "Modificar Nota de Débito";
            ListarCombos();
            dgvDetalle.AutoGenerateColumns = false;
            if (codigoNotaDebito.HasValue)
            {
                CargarNotaDebito();
            }
            else
        ...
        void CargarNotaDebito()
        {
            NotaDebitoBe item = notaDebitoBl.ObtenerNotaDebito(codigoNotaDebito.Value, true, withUnidadMedida: true);
            ...
            listaDetalleInicial = item.ListaNotaDebitoDetalle;
            listaDetalle = item.ListaNotaDebitoDetalle;
            ListarNotaDebitoDetalle();
        }
        ...
        NotaDebitoDetalleBe CargarListaDetalleRef(FacturaVentaDetalleBe item, FacturaVentaBe comprobante)
        {
            NotaDebitoDetalleBe itemNotaDebito = new NotaDebitoDetalleBe();
            itemNotaDebito.CodigoTipoComprobanteRef = (int)TipoComprobante.Factura;
            itemNotaDebito.CodigoComprobanteRef = comprobante.CodigoFacturaVenta;
            itemNotaDebito.CodigoComprobanteDetalleRef = item.CodigoFacturaVentaDetalle;
            itemNotaDebito.CodigoProducto = item.CodigoProducto;
            itemNotaDebito.Producto = productoBl.ObtenerProducto(item.CodigoProducto);
            itemNotaDebito.CodigoProductoIndividual = item.CodigoProductoIndividual;
            itemNotaDebito.ProductoIndividual = productoIndividualBl.ObtenerProductoIndividual(item.CodigoProductoIndividual);
            itemNotaDebito.CodigoUnidadMedida = itemNotaDebito.ProductoIndividual.CodigoUnidadMedida;
            itemNotaDebito.UnidadMedida = unidadMedidaBl.ObtenerUnidadMedida(itemNotaDebito.CodigoUnidadMedida);
            itemNotaDebito.Detalle = item.Detalle;
            itemNotaDebito.Cantidad = item.Cantidad;
            itemNotaDebito.PrecioUnitario = item.PrecioUnitario;
            itemNotaDebito.TipoCalculo = (int)TipoCalculo.PrecioUnitario;
            itemNotaDebito.TipoDescuento = (int)TipoDescuento.Descuento;
            Calculo(itemNotaDebito);

            return itemNotaDebito;
        }

        NotaDebitoDetalleBe CargarListaDetalleRef(BoletaVentaDetalleBe item, BoletaVentaBe comprobante)
        {
            NotaDebitoDetalleBe itemNotaDebito = new NotaDebitoDetalleBe();
            itemNotaDebito.CodigoTipoComprobanteRef = (int)TipoComprobante.Boleta;
            itemNotaDebito.CodigoComprobanteRef = comprobante.CodigoBoletaVenta;
            itemNotaDebito.CodigoComprobanteDetalleRef = item.CodigoBoletaVentaDetalle;
            itemNotaDebito.CodigoProducto = item.CodigoProducto;
            itemNotaDebito.Producto = productoBl.ObtenerProducto(item.CodigoProducto);
            itemNotaDebito.CodigoProductoIndividual = item.CodigoProductoIndividual;
            itemNotaDebito.ProductoIndividual = productoIndividualBl.ObtenerProductoIndividual(item.CodigoProductoIndividual);
        ...
        void Calculo(NotaDebitoDetalleBe item)
        {
        ...
        void ReCalcularDetalleComprobanteRef(TipoComprobante? tipoComprobante, dynamic comprobanteRef)
        {
            ...
            if (comprobanteRef != null)
            {
                ...
                {
                    NotaDebitoDetalleBe itemNotaDebito = CargarListaDetalleRef(item, comprobanteRef);

                    listaDetalle.Add(itemNotaDebito);
                }
        ...
        {
            List<SerieBe> listaCombo = serieBl.ListarComboSerie((int)TipoComprobante.NotaDebito);
        ...
        {
            List<MotivoNotaBe> listaCombo = motivoNotaBl.ListarComboMotivoNota((int)TipoComprobante.NotaDebito);
        ...
        void CargarComprobanteRef(int? codigoComprobante)
        {
            ...
            {
                dynamic comprobante = tipoComprobanteRef == TipoComprobante.Factura ? (dynamic)facturaVentaBl.ObtenerFacturaVenta(codigoComprobante.Value, true, withSerie: true) : tipoComprobanteRef == TipoComprobante.Boleta ? (dynamic)boletaVentaBl.ObtenerBoletaVenta(codigoComprobante.Value, true, true) : null;
                if (comprobante != null)
                {
        ...
        {
            FrmMantenimientoNotaDebitoDetalle frm = new FrmMantenimientoNotaDebitoDetalle();
            frm.ShowInTaskbar = false;
            frm.BringToFront();
            ...
            if (dr == DialogResult.OK)
            {
                frm.Detalle.CodigoNotaDebitoDetalle = GenerarCodigoNotaDebitoDetalleTemporal(frm.Detalle.CodigoProductoIndividual);
        ...
        private void btnModificarDetalle_Click(object sender, EventArgs e)
        {
            ...
            var item = (NotaDebitoDetalleBe)dgvDetalle.Rows[rowIndex].DataBoundItem;
            int indexData = listaDetalle.IndexOf(item);

            FrmMantenimientoNotaDebitoDetalle frm = new FrmMantenimientoNotaDebitoDetalle(item);
            frm.ShowInTaskbar = false;
            frm.BringToFront();
            frm.ListaDetalleInicial = this.listaDetalleInicial;
            frm.ListaDetalleActual = this.listaDetalle;
        ...
        {
            listaDetalle = listaDetalle ?? new List<NotaDebitoDetalleBe>();
        ...
        private void btnGuardar_Click(object sender, EventArgs e)
        {
            ...
            NotaDebitoBe registro = new NotaDebitoBe();

            if (codigoNotaDebito.HasValue) registro.CodigoNotaDebito = codigoNotaDebito.Value;
            ...
            registro.ListaNotaDebitoDetalle = listaDetalle;
            registro.ListaNotaDebitoDetalleEliminar = listaDetalleInicial == null ? null : listaDetalleInicial.Where(x => listaDetalle.Count(y => y.CodigoNotaDebitoDetalle == x.CodigoNotaDebitoDetalle) == 0).Select(x => x.CodigoNotaDebitoDetalle).ToArray();
            ...
            bool seGuardoRegistro = notaDebitoBl.GuardarNotaDebito(registro, out nroComprobante, out totalEnLetras);
            ...
            if (seGuardoRegistro)
            {
                ...
                if (Emision.ExisteConexaoInternet() == true)
                {
                    ...
                    //Genera el xml
                    DocumentoNDElectronicos(registro, out archivoXML);
                    if (!string.IsNullOrEmpty(archivoXML))
                    {
                        ...
                        seGuardoRegistro = notaDebitoBl.GuardarEmisionNotaDebito(registro);
                    }
                }
                else
                {
        ...
        bool ValidarFormulario()
        {
            ...
            if ((listaDetalle ?? new List<NotaDebitoDetalleBe>()).Count == 0)
            {
                estaValidado = false;
        ...
        {
            tltNotaDebito.SetToolTip(label, label.Text);
        }
        ...
        int GenerarCodigoNotaDebitoDetalleTemporal(int codigoProductoIndividual)
        {
            int codigoNotaDebitoDetalle = 0;
            listaDetalleInicial = listaDetalleInicial ?? new List<NotaDebitoDetalleBe>();
            var registro = listaDetalleInicial.FirstOrDefault(x => x.CodigoProductoIndividual == codigoProductoIndividual);
            listaDetalle = listaDetalle ?? new List<NotaDebitoDetalleBe>();
            if (registro == null) codigoNotaDebitoDetalle = listaDetalle.Select(x => x.CodigoNotaDebitoDetalle).DefaultIfEmpty().Min() - 1;
            else codigoNotaDebitoDetalle = registro.CodigoNotaDebitoDetalle;
            return codigoNotaDebitoDetalle;
        }

        void DocumentoNDElectronicos(NotaDebitoBe item, out string archivoXML)
        {
            archivoXML = null;
            ...
            string codigoSunatTipoComprobante = TipoComprobante.NotaDebito.GetAttributeOfType<CategoryAttribute>().Category;
            ...
            debitNote.Xmlns = EspacioNombres.xmlnsDebitNote;
            debitNote.Cac = EspacioNombres.cac;
            debitNote.Cbc = EspacioNombres.cbc;
            debitNote.Ccts = EspacioNombres.ccts;
            debitNote.Ds = EspacioNombres.ds;
            debitNote.Ext = EspacioNombres.ext;
            debitNote.Qdt = EspacioNombres.qdt;
            debitNote.Udt = EspacioNombres.udt;
            debitNote.sac = EspacioNombres.sac;
            // debitNote.Xsi = EspacioNombres.xsi;


            XmlDocument xmlDocument = new XmlDocument();
            XmlElement firma = xmlDocument.CreateElement("ext:firma", debitNote.Ext);
            ...
            debitNote.UBLExtensions = uBLExtensiones;
            debitNote.UBLVersionID = new UBLVersionIDType();
            debitNote.UBLVersionID.Value = "2.1";

            debitNote.CustomizationID = new CustomizationIDType();
            debitNote.CustomizationID.Value = "2.0";


            //numeracion conformado por serie correlativo
            debitNote.ID = new IDType();

            debitNote.ID.Value = item.Serie.Serial + "-" + item.NroComprobante.ToString("00000000");

            //fecha de emision y hora de emision
            debitNote.IssueDate = new IssueDateType();
            debitNote.IssueDate.Value = item.FechaHoraEmision;

            debitNote.IssueTime = new IssueTimeType();
            debitNote.IssueTime.Value = Convert.ToDateTime(String.Format("{0}:{1}:{2}", item.FechaHoraEmision.Hour, item.FechaHoraEmision.Minute, item.FechaHoraEmision.Second));
            ...
            debitNote.DocumentCurrencyCode = Moneda;
            ...
            debitNote.DiscrepancyResponse = new ResponseType[] { nResponseType };
            ...
            debitNote.BillingReference = new BillingReferenceType[] { bill };
            //-------------------------------------------------------------------------------------------
            ...
            party.PartyName = partynames2;
            Empresa.Party = party;
            debitNote.AccountingSupplierParty = Empresa;
            ...
            debitNote.AccountingCustomerParty = CustomerPartyCliente;
            ...
            debitNote.RequestedMonetaryTotal = totalvalorventa;
            ...
            int contador = item.ListaNotaDebitoDetalle.Count + 1;

            DebitNoteLineType[] items = new DebitNoteLineType[contador];
            ...
            foreach (var detalle in item.ListaNotaDebitoDetalle)
            {
                ...
                DebitNoteLineType deditNoteLine = new DebitNoteLineType();

                IDType NumeroItem = new IDType();
                NumeroItem.Value = Convert.ToString(iditemnumero);
                deditNoteLine.ID = NumeroItem;
                ...
                deditNoteLine.DebitedQuantity = cantidad;
                ...
                deditNoteLine.LineExtensionAmount = valorventa;
                ...
                deditNoteLine.PricingReference = ValorReferenUnitario;
                ...
                deditNoteLine.TaxTotal = totales_items;
                ...
                deditNoteLine.Item = itemTipo;
                deditNoteLine.Price = PrecioProducto;


                items[Convert.ToInt32(iditemnumero)] = deditNoteLine;

            }

            debitNote.DebitNoteLine = items;

            archivoXML = GenerarComprobanteNotaDebito(ref debitNote, rucEmpresa, codigoSunatTipoComprobante, item.Serie.Serial, item.NroComprobante.ToString("00000000"));


        }

        public string GenerarComprobanteNotaDebito(ref DebitNoteType FactXML, string EmpresaRuc, string TipoFactura, string Fac_serie, string NumFactura)
        {
            XmlSerializer nXmlSerializer = new XmlSerializer(typeof(DebitNoteType));
            XmlWriter xtWriter;
            XmlWriterSettings setting = new XmlWriterSettings();
            setting.Indent = true;
            setting.IndentChars = "\t";
            string RutaDocumento = Path.Combine(configuracionValor.RutaFacturacionElectronica, "NOTA DEBITO");
        ...
        public void EnviarDocumento(string FileArchivo, string ArchXML, out string CodigoHashRpta, out string DescripcionRpta, out string CodigoRpta)
        {
            ...
            try
            {
                ...
                //  NotaDebitoBe.ModificarHast(registro);
