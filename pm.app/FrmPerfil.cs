...
namespace pm.app
{
    public partial class FrmPerfil : RadForm
    {
        ...
        void BuscarPerfiles()
        {
            ...
            List<PerfilBe> resultados = perfilBl.BuscarPerfil(nombre, flagActivo);
        ...
        {
            FrmMantenimientoPerfil frm = new FrmMantenimientoPerfil();
            frm.ShowInTaskbar = false;
            frm.BringToFront();
            DialogResult dr = frm.ShowDialog();
            if (dr == DialogResult.OK) BuscarPerfiles();
        }
        ...
        private void dgvResultados_MouseClick(object sender, MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Right)
            {
                ...
                int codigoPerfil = (int)dgvResultados.Rows[currentMouseOverRow].Cells["dgvResultados_CodigoPerfil"].Value;

                ContextMenu m = new ContextMenu();
                MenuItem mitEditar = new MenuItem("Editar", mitEditar_Click);
                mitEditar.Tag = codigoPerfil;

                MenuItem mitToggleActivar = new MenuItem(flagActivo ? "Inactivar" : "Activar", mitToggleActivar_Click);
                mitToggleActivar.Tag = new { CodigoPerfil = codigoPerfil, FlagActivo = flagActivo };
        ...
        private void mitEditar_Click(object sender, EventArgs e)
        {
            ...
            int codigoPerfil = (int)mitControl.Tag;

            FrmMantenimientoPerfil frm = new FrmMantenimientoPerfil(codigoPerfil);
            frm.ShowInTaskbar = false;
            frm.BringToFront();
            DialogResult dr = frm.ShowDialog();
            if (dr == DialogResult.OK) BuscarPerfiles();
